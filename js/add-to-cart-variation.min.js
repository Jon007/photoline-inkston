(function($,window,document,undefined){var VariationForm=function($form){this.$form=$form;this.$attributeFields=$form.find('.variations select');this.$singleVariation=$form.find('.single_variation'),this.$singleVariationWrap=$form.find('.single_variation_wrap');this.$resetVariations=$form.find('.reset_variations');this.$product=$form.closest('.product');this.variationData=$form.data('product_variations');this.useAjax=!1===this.variationData;this.xhr=!1;this.$singleVariationWrap.show();this.$form.unbind('check_variations update_variation_values found_variation');this.$resetVariations.unbind('click');this.$attributeFields.unbind('change ');this.getChosenAttributes=this.getChosenAttributes.bind(this);this.findMatchingVariations=this.findMatchingVariations.bind(this);this.isMatch=this.isMatch.bind(this);this.toggleResetLink=this.toggleResetLink.bind(this);$form.on('click','.reset_variations',{variationForm:this},this.onReset);$form.on('reload_product_variations',{variationForm:this},this.onReload);$form.on('hide_variation',{variationForm:this},this.onHide);$form.on('show_variation',{variationForm:this},this.onShow);$form.on('click','.single_add_to_cart_button',{variationForm:this},this.onAddToCart);$form.on('reset_data',{variationForm:this},this.onResetDisplayedVariation);$form.on('reset_image',{variationForm:this},this.onResetImage);$form.on('change','.variations select',{variationForm:this},this.onChange);$form.on('found_variation',{variationForm:this},this.onFoundVariation);$form.on('check_variations',{variationForm:this},this.onFindVariation);$form.on('update_variation_values',{variationForm:this},this.onUpdateAttributes);$form.trigger('check_variations');$form.trigger('wc_variation_form')};VariationForm.prototype.onReset=function(event){event.preventDefault();event.data.variationForm.$attributeFields.val('').change();event.data.variationForm.$form.trigger('reset_data')};VariationForm.prototype.onReload=function(event){var form=event.data.variationForm;form.variationData=form.$form.data('product_variations');form.useAjax=!1===form.variationData;form.$form.trigger('check_variations')};VariationForm.prototype.onHide=function(event){event.preventDefault();event.data.variationForm.$form.find('.single_add_to_cart_button').removeClass('wc-variation-is-unavailable').addClass('disabled wc-variation-selection-needed');event.data.variationForm.$form.find('.woocommerce-variation-add-to-cart').removeClass('woocommerce-variation-add-to-cart-enabled').addClass('woocommerce-variation-add-to-cart-disabled')};VariationForm.prototype.onShow=function(event,variation,purchasable){event.preventDefault();if(purchasable){event.data.variationForm.$form.find('.single_add_to_cart_button').removeClass('disabled wc-variation-selection-needed wc-variation-is-unavailable');event.data.variationForm.$form.find('.woocommerce-variation-add-to-cart').removeClass('woocommerce-variation-add-to-cart-disabled').addClass('woocommerce-variation-add-to-cart-enabled')}else{event.data.variationForm.$form.find('.single_add_to_cart_button').removeClass('wc-variation-selection-needed').addClass('disabled wc-variation-is-unavailable');event.data.variationForm.$form.find('.woocommerce-variation-add-to-cart').removeClass('woocommerce-variation-add-to-cart-enabled').addClass('woocommerce-variation-add-to-cart-disabled')}};VariationForm.prototype.onAddToCart=function(event){if($(this).is('.disabled')){event.preventDefault();if($(this).is('.wc-variation-is-unavailable')){window.alert(wc_add_to_cart_variation_params.i18n_unavailable_text)}else if($(this).is('.wc-variation-selection-needed')){window.alert(wc_add_to_cart_variation_params.i18n_make_a_selection_text)}}};VariationForm.prototype.onResetDisplayedVariation=function(event){var form=event.data.variationForm;form.$product.find('.product_meta').find('.sku').wc_reset_content();form.$product.find('.product_weight').wc_reset_content();form.$product.find('.product_dimensions').wc_reset_content();form.$product.find('.woocommerce-variation-custom-asin').wc_reset_content();form.$product.find('.woocommerce-variation-custom-_sku').wc_reset_content();form.$product.find('.woocommerce-variation-custom-upc').wc_reset_content();form.$form.trigger('reset_image');form.$singleVariation.slideUp(200).trigger('hide_variation')};VariationForm.prototype.onResetImage=function(event){event.data.variationForm.$form.wc_variations_image_update(!1)};VariationForm.prototype.onFindVariation=function(event){var form=event.data.variationForm,attributes=form.getChosenAttributes(),currentAttributes=attributes.data;if(attributes.count===attributes.chosenCount){if(form.useAjax){if(form.xhr){form.xhr.abort()}
...
form.toggleResetLink(attributes.chosenCount>0)};VariationForm.prototype.onFoundVariation=function(event,variation){var form=event.data.variationForm,$sku=form.$product.find('.product_meta').find('.sku'),$weight=form.$product.find('.product_weight'),$dimensions=form.$product.find('.product_dimensions'),$asin=form.$product.find('.woocommerce-variation-custom-asin'),$skud=form.$product.find('.woocommerce-variation-custom-_sku'),$upc=form.$product.find('.woocommerce-variation-custom-upc'),$netweight=form.$product.find('.woocommerce-variation-custom-net_weight'),$netsize=form.$product.find('.woocommerce-variation-custom-net_size'),$qty=form.$singleVariationWrap.find('.quantity'),purchasable=!0,variation_id='',template=!1,$template_html='';if(variation.sku){$sku.wc_set_content(variation.sku);$skud.wc_set_content(variation.sku)}else{$sku.wc_reset_content()}
if(variation.weight){$weight.wc_set_content(variation.weight_html)}else{$weight.wc_reset_content()}
if(variation.net_weight){$netweight.wc_set_content(variation.net_weight)}else{$netweight.wc_reset_content()}
if(variation.net_size){$netsize.wc_set_content(variation.net_size)}else{$netsize.wc_reset_content()}
if(variation.dimensions){$dimensions.wc_set_content(variation.dimensions_html)}else{$dimensions.wc_reset_content()}
if(variation.asin){$asin.wc_set_content(variation.asin)}else{$asin.wc_reset_content()}
if(variation.upc){$upc.wc_set_content(variation.upc)}else{$asin.wc_reset_content()}
...
attached_options_count=new_attr_select.find('option.attached').length;if(selected_attr_val&&(attached_options_count===0||new_attr_select.find('option.attached.enabled[value="'+form.addSlashes(selected_attr_val)+'"]').length===0)){selected_attr_val_valid=!1}
