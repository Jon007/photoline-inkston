/* inkston additional variation fields, updated 24/july to woo3.1+ */
...
;(function ( $, window, document, undefined ) {
    ...
    form.$product.find( '.woocommerce-variation-custom-net_weight' ).wc_reset_content();
		form.$product.find( '.woocommerce-variation-custom-net_size' ).wc_reset_content();
	...
	VariationForm.prototype.onFoundVariation = function( event, variation ) {
		var form           = event.data.variationForm,
			...
			$netsize			 = form.$product.find( '.woocommerce-variation-custom-net_size' ),	
...
//			debugger;
		if ( variation.sku ) {
			...
			$skud.wc_set_content( variation.sku );
		} else {
			$sku.wc_reset_content();
			$skud.wc_reset_content();
		}
		...
		if ( variation.net_weight ) {
			$netweight.wc_set_content( variation.net_weight );
		} else {
			$netweight.wc_reset_content();
		}

		if ( variation.net_size ) {
			$netsize.wc_set_content( variation.net_size );
		} else {
			$netsize.wc_reset_content();
		}
		...
		if ( variation.asin ) {
			$asin.wc_set_content( variation.asin );
		} else {
