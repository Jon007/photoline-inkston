 ...
 */
add_action('widgets_init', create_function('', 'register_widget("inkston_Icon_Widget");'));

class inkston_Icon_Widget extends WP_Widget {
	function __construct() {
		parent::__construct(
			'inkston_icon_widget',
			'inkston - ' . __( 'Icons', 'photoline-inkston' ),
			array(
				'classname' => 'inkston_icon_widget', 
				'description' => __( 'Custom Text Widget with Icons', 'photoline-inkston' ),
				'width' => 250,
				'height' => 350
			)
		);
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'photoline-inkston' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></p>

		<p><label><?php _e( 'Icon:', 'photoline-inkston' ); ?></label>
		<span class="custom-icon-container"><?php echo $icon_tag; ?></span>
		<a href="#" class="view-icons"><?php _e( 'View Icons', 'photoline-inkston' ); ?></a> | <a href="#" class="delete-icon"><?php _e( 'Remove', 'photoline-inkston' ); ?></a>
		<?php inkston_font_awesome_icons(); ?>
		<input class="image-widget-custom-icon" name="<?php echo $this->get_field_name( 'icon' ); ?>" type="hidden" value="<?php echo esc_attr( $icon ); ?>" /></p>

		<p><label for="<?php echo $this->get_field_id( 'url' ); ?>"><?php _e( 'URL:', 'photoline-inkston' ); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'button_text' ); ?>"><?php _e( 'Button Text:', 'photoline-inkston' ); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'button_color' ); ?>"><?php _e( 'Icon and Button Color:', 'photoline-inkston' ); ?></label>
		<select name="<?php echo $this->get_field_name( 'button_color' ); ?>" id="<?php echo $this->get_field_id( 'button_color' ); ?>" class="widefat">
		<?php
		$options = array(
			'default' => __( 'Default', 'photoline-inkston' ),
			'blue' => __( 'Blue', 'photoline-inkston' ),
			'red' => __( 'Red', 'photoline-inkston' ),
			'green' => __( 'Green', 'photoline-inkston' ),
		);
		...
		<p><input id="<?php echo $this->get_field_id('filter'); ?>" name="<?php echo $this->get_field_name('filter'); ?>" type="checkbox" <?php checked( isset( $filter ) ? $filter : 0 ); ?> />&nbsp;<label for="<?php echo $this->get_field_id('filter'); ?>"><?php _e( 'Automatically add paragraphs', 'photoline-inkston' ); ?></label></p>
		<?php
	}
}