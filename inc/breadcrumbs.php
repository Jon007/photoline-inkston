 ...
 * @package Inkston
 */
if ( !function_exists( 'inkston_breadcrumb' ) ) {
	function inkston_breadcrumb() {
        if (is_search()){
          if (get_search_query()==''){
            echo('<div id="breadcrumb">Inkston - Search</div>');
            return;
          }
        }
        /* very simply, output the yoast breadcrumb if it exists */
        if ( function_exists('yoast_breadcrumb') ) {
            /* default from yoast:  yoast_breadcrumb('<p id="breadcrumbs">', '</p>'); */
            yoast_breadcrumb('<div id="breadcrumb">', '</div>');
            return;
        }
        else {
            echo ('<!-- DEBUG: Yoast breadcrumb turned off -->');
        }


		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'photoline-inkston' ); // text for the 'Home' link
		$text['category'] = __( 'Archive for %s', 'photoline-inkston' ); // text for a category page
		$text['search']   = __( 'Search results for: %s', 'photoline-inkston' ); // text for a search results page
		$text['tag']      = __( 'Posts tagged %s', 'photoline-inkston' ); // text for a tag page
		$text['author']   = __( 'View all posts by %s', 'photoline-inkston' ); // text for an author page
		$text['404']      = __( 'Error 404', 'photoline-inkston' ); // text for the 404 page
		...
		if ( is_front_page() ) {
			...
			if ( get_query_var( 'paged' ) || get_query_var( 'page' ) ) {
				...
				$html_output .= $delimiter . sprintf( __( 'Page %s', 'photoline-inkston' ), $page_num );
